...
int main(int argc, char** argv) {
    ...
    MPI_Cart_coords(grid_comm, local_grid_rank, 3, coordinates);
    MPI_Cart_rank(grid_comm, coordinates, &grid_rank);

    printf("Process %d local_grid_rank = %d, coords = (%d, %d, %d), grid_rank = %d\n", rank, local_grid_rank, coordinates[0], coordinates[1], coordinates[3], grid_rank);
